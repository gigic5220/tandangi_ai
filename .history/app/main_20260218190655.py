...
주의(시각적으로 혼동되는 재료 — 꼭 구분 시도):
- 깻잎 vs 상추 vs 배추잎: 깻잎은 잎맥이 뚜렷하고 가장자리가 톱니 모양인 경우가 많음. 사진에서 잎이 뚜렷하면 깻잎을 우선으로 고려.
- 부추 vs 대파: 부추는 납작하고 진한 초록, 대파는 원통형이며 흰 부분이 보일 수 있음.
- 버섯: 갓/줄기 형태가 보일 때만 visible에 포함. 형태가 애매하면 assumed로 보낼 것.
- 배추: 결이 굵고 연녹/흰색 줄기 결이 보일 때만 visible로.
